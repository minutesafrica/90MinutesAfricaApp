...
public class CompetitionsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AdView competitionsBannerAd = findViewById(R.id.competitionsBannerAd);
        competitionsBannerAd.loadAd(new AdRequest.Builder().build());


        findViewById(R.id.competitionsBack).setOnClickListener(v -> finish());

        newsContainer = findViewById(R.id.competitionsNewsContainer);
    ...
    private void displayNews(JSONArray news) {
        ...
        try {
            for (int i = 0; i < news.length(); i++) {
                ...
                if (!category.equalsIgnoreCase("MASHINDANO")) {
                    continue;
                }
            ...
            if (count == 0) {
                showMessage("Hakuna habari za MASHINDANO kwa sasa.");
            }
    ...
    private void addNewsCard(JSONObject item) {
        ...
        card.setOnClickListener(v -> {
            Intent intent = new Intent(CompetitionsActivity.this, ArticleDetailActivity.class);
