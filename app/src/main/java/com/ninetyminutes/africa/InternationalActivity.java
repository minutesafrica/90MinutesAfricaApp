...
public class InternationalActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AdView internationalBannerAd = findViewById(R.id.internationalBannerAd);
        internationalBannerAd.loadAd(new AdRequest.Builder().build());


        findViewById(R.id.internationalBack).setOnClickListener(v -> finish());

        newsContainer = findViewById(R.id.internationalNewsContainer);
    ...
    private void displayNews(JSONArray news) {
        ...
        try {
            for (int i = 0; i < news.length(); i++) {
                ...
                if (!category.equalsIgnoreCase("KIMATAIFA")) {
                    continue;
                }
            ...
            if (count == 0) {
                showMessage("Hakuna habari za KIMATAIFA kwa sasa.");
            }
    ...
    private void addNewsCard(JSONObject item) {
        ...
        card.setOnClickListener(v -> {
            Intent intent = new Intent(InternationalActivity.this, ArticleDetailActivity.class);
