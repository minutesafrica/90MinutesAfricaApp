...
public class TransfersActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AdView transfersBannerAd = findViewById(R.id.transfersBannerAd);
        transfersBannerAd.loadAd(new AdRequest.Builder().build());


        findViewById(R.id.transfersBack).setOnClickListener(v -> finish());

        newsContainer = findViewById(R.id.transfersNewsContainer);
    ...
    private void displayNews(JSONArray news) {
        ...
        try {
            for (int i = 0; i < news.length(); i++) {
                ...
                if (!category.equalsIgnoreCase("USAJILI")) {
                    continue;
                }
            ...
            if (count == 0) {
                showMessage("Hakuna habari za USAJILI kwa sasa.");
            }
    ...
    private void addNewsCard(JSONObject item) {
        ...
        card.setOnClickListener(v -> {
            Intent intent = new Intent(TransfersActivity.this, ArticleDetailActivity.class);
