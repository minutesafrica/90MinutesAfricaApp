...
public class ClubsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AdView clubsBannerAd = findViewById(R.id.clubsBannerAd);
        clubsBannerAd.loadAd(new AdRequest.Builder().build());


        findViewById(R.id.clubsBack).setOnClickListener(v -> finish());

        newsContainer = findViewById(R.id.clubsNewsContainer);
    ...
    private void displayNews(JSONArray news) {
        ...
        try {
            for (int i = 0; i < news.length(); i++) {
                ...
                if (!category.equalsIgnoreCase("VILABU")) {
                    continue;
                }
            ...
            if (count == 0) {
                showMessage("Hakuna habari za VILABU kwa sasa.");
            }
    ...
    private void addNewsCard(JSONObject item) {
        ...
        card.setOnClickListener(v -> {
            Intent intent = new Intent(ClubsActivity.this, ArticleDetailActivity.class);
